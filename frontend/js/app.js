...
async function renderStudents() {
  ...
  els.viewRoot.querySelectorAll("[data-view-student]").forEach((b) => b.addEventListener("click", async () => {
    const id = b.dataset.viewStudent;
    const row = d.items.find((x) => String(x.id) === String(id));
    const fees = await api.listFees(qs({ search: row.name, page: 1, page_size: 50 }));
    els.viewRoot.innerHTML = `<section class="panel"><h3>Student Profile: ${row.name}</h3>
      <p>Class: ${row.class_name}</p><p>Phone: ${row.phone}</p><p>Address: ${row.address}</p>
      <h4>Payment History</h4>${table(["Amount", "Month", "Date"], (fees.data.items || []).map((f) => `<tr><td>${f.amount}</td><td>${f.month}</td><td>${formatHumanDate(f.payment_date)}</td></tr>`))}
      <h4>Behavior Notes</h4><p>${row.notes || "None"}</p>
      <h4>Attendance Summary</h4><p>Use Attendance tab for date/class specific records.</p>
      <h4>Class History Log</h4><p>${row.class_name}</p>
...
async function renderStaff() {
  ...
  els.viewRoot.querySelectorAll("[data-view-staff]").forEach((b) => b.addEventListener("click", () => {
    const s = JSON.parse(b.dataset.viewStaff);
    els.viewRoot.innerHTML = `<section class='panel'><h3>Staff Profile: ${s.name}</h3><p>Role: ${s.role}</p><p>Salary: ${s.salary}</p><p>Phone: ${s.phone}</p><p>Status: ${s.status}</p><h4>Salary Payment History</h4><p>Use Finance module records filtered by staff payments.</p><h4>Performance Notes</h4><p>Available in future extension.</p><h4>Attendance Record</h4><p>Use Attendance tab with class/date.</p><button class='btn' id='backStaff'>Back</button></section>`;
