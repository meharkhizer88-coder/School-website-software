...
def student_profile(student_id: int, x_role: str = Header(default="Super Admin")):
    authorize(x_role, "manage_students")
    data = svc.student_profile(student_id)
    if not data:
        raise HTTPException(status_code=404, detail="Student not found")
    return ApiResponse(success=True, data=data, message="Student profile loaded")
...
def edit_staff(staff_id: int, payload: dict, x_role: str = Header(default="Super Admin")):
    authorize(x_role, "manage_staff")
    svc.update_staff(staff_id, payload)
    return ApiResponse(success=True, data={"id": staff_id}, message="Staff updated")
...
def staff_profile(staff_id: int, x_role: str = Header(default="Super Admin")):
    authorize(x_role, "manage_staff")
    data = svc.staff_profile(staff_id)
    if not data:
        raise HTTPException(status_code=404, detail="Staff not found")
    return ApiResponse(success=True, data=data, message="Staff profile loaded")
